...
**generate_html.py** — 把 knowledge JSON 渲染为交互式 HTML。模板存在 `references/html-template.md` 里（用 ```html 代码块包裹），`extract_html_from_markdown` 把它提取出来，替换 `__DATA_PLACEHOLDER__` / `[教材名称]` / `[时间戳]` 占位符。防 `</script>` 注入：把 `</` 转成 `<\/`，浏览器 JSON.parse 自动还原。
...
### 4. 为什么 HTML 模板塞在 .md 里

当前 `references/html-template.md` 用 ```html 代码块包裹 HTML 模板，`generate_html.py` 用正则提取。好处：宿主 LLM 读 `.md` 文件天然就能看懂模板内容，不用解析额外文件类型。**注：即将外置为独立 `.html` 文件**——模板越来越大，`.md` 包裹的优势减弱，独立 HTML 文件更便于编辑器语法高亮和版本对比。
