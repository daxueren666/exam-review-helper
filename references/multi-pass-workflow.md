...
输出：
- `Review - input/extracted_content.md`（PDF 全文 markdown）
- `Review - input/extracted_content.json`（docling 结构化数据，含 pages 数组）

**Page 标注**：markdown 中应包含 `[PAGE N]` 标记。若无（docling 旧版本），从 `extracted_content.json` 的 pages 数组推断总页数。

**失败处理**：
- docling 装不上 → 提示用户用 PyMuPDF 兜底（功能受限，仅文字版可用）
- OCR 质量差 → 警告用户但不中止
