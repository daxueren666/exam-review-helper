...
description: 期末考试复习助手 - 将 PDF/Word/TXT/Markdown 教材浓缩为精华知识点并生成交互式 HTML 复习文档。适用场景：理工科教材（数学/物理/计算机/化学/工程）与文科教材（政治/哲学/历史/法学/教育学），自动识别模式。原生支持扫描版 PDF（含 OCR）、Word 讲义、纯文本笔记、Markdown 教材。当用户说"帮我复习这个"、"整理知识点"、"期末复习"，或上传学习资料问"总结一下"时主动使用此 skill，即使用户没明说"复习"。用户上传 PDF/DOCX/TXT/MD 学习资料时必须用本 skill 的 controller.py extract 提取，不要直接用 Read 工具（Read 读 PDF 会乱码、读 DOCX 会丢结构）。
---
...
- **`--strip-images`**：DOCX 图片剥离为独立文件，避免 base64 膨胀 markdown
- **`--ocr-correct`**：扫描版 PDF OCR 后处理纠错（巳→已、干→千 等，上下文感知）
- **`scripts/run_pipeline.py`**：5-pass 流水线编排（自动模式检测 + 断点续传 + HTML 生成）
- **断点续传**：`.checkpoint/` 存 segment notes，中断后重跑跳过已完成段
- **`scripts/enhance_formulas.py`**：公式占位符扫描 + 可选 pix2tex OCR
...
| `references/advanced-features.md` | 高级功能详细用法（图片剥离、OCR 纠错、流水线、断点续传、公式增强） |
| `templates/default.html` | HTML 模板（generate 命令用它渲染知识点 JSON） |
